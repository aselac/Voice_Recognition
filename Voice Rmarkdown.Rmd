...
We see a correlation in certain predictors. Lets see what predictors are correlated.We can put values over 0.7 as highly correlated. 
```{r,echo=TRUE, warning=FALSE}
...
## Data splitting
Data set will be split into 30% for testing and 70% for training.
```{r,echo=TRUE, warning=FALSE}
...
The KNN algorithm assumes that similar things exist in close proximity. Lets apply the KNN model and see the accuracy.
...
knn
```
It provides accuracy of 97% with best value for K= 5. 
